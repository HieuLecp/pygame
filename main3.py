...
#Lỗi 2 ảnh ko có trong file
CHICKEN = pygame.image.load(os.path.join("assets", "egg.png"))
CHICKEN = pygame.transform.scale(CHICKEN, (20, 40))
ROCKET = pygame.image.load(os.path.join("assets", "egg.png"))
ROCKET = pygame.transform.scale(ROCKET, (40, 40))
ROCKET_LASER = pygame.transform.scale(ROCKET, (500, 500))
...
#draw: Vẽ lên màn hình obj
#move: Di chuyển theo tốc độ "vel"
#off_screen: Out khỏi màn hình 
#collision: 2 obj chạm nhau thì collide -> xóa khỏi màn hình
...
class Plan:
    ...
    def add_rocket(self):
        if self.cool_down_counter == 0:
            rocket = Egg(self.x - 150, self.y, ROCKET_LASER)
...
class Boss(Plan):   
    ...
    # move kiểu ko out khỏi màn hình mà bay xung quanh lung tung...
    def move(self):
...
class Drop_box():
    ...
    # rơi tốc độ "vel", random màu, random vị trí x(ngang)
    def drop(self, vel):
        self.y += vel   
        if self.y >= HEIGHT:
...
def main():
    ...
    def redraw_window():
        ...
        #Nếu player ăn dc box_bullet thì health + 20, đổi đạn
        ...
        rocket_label = main_font.render(f": {rocket > 0}", 1, (255,255,255))
        ...
        if level == 4:
            ...
            player.shoot_rocket(-laser_vel_player, boss) 
            if boss.health == 0:
    ...
    while run:
        ...
        if keys[pygame.K_SPACE] :
            player.shoot()
        #P thì thêm rocket là 1 Egg() vào rocket của player, số lượng rocket ở ngoài - 1
        if keys[pygame.K_p] :
            if rocket > 0:
                player.add_rocket()
                rocket = rocket - 1
        ...
        for chicken_thigh in player.chicken:
            chicken_thigh.move(chicken_thigh_vel)  
